          ...
          <div v-show="selecteds.length > 0">
            ...
            <template v-else>
              <el-tag closable
                      ...
                      @close="tabClose(collapseTagsItem[nodeKey])">{{ collapseTagsItem[selfProps.label] }}</el-tag>
              <el-tag v-if="this.selecteds.length > 1"
                      :size="size"
                      class="wl-select-tag">+{{ this.selecteds.length - 1 }}</el-tag>
            </template>
          </div>
          <p class="wl-placeholder-box"
...
export default {
  ...
  methods: {
    ...
    handleCheckChange(val, { checkedNodes }) {
      ...
      if (checkedNodes.length === 0 && this.noCheckedClose)
