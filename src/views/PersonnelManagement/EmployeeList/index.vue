...
export default {
  ...
  methods: {
    ...
    // 搜索
    searchList() {
      this.queryParams.unit_id_list = [];
      this.superiorUnit.map((item) => {
        this.queryParams.unit_id_list.push(item["unit_code"]);
      });
      let param = {
        page: 1,
        page_size: 10,
        department_id_list: this.queryParams.department_id_list,
        duty_id_list: this.queryParams.duty_id_list,
        unit_id_list: this.queryParams.unit_id_list,
      };
      if (this.queryParams.search_name) {
        param["search_name"] = this.queryParams.search_name;
      }
      console.log(param);
      GET_USER_LIST(param).then((res) => {
        if (res.status === 0) {
